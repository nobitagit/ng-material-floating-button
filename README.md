...
###More customisations###
The component have plenty more customisations available and they are all handled by the CSS. The CSS and its SCSS source files are found in the `mfb/src` folder (which is actually a subtree that pulls from this [repo](https://github.com/nobitagit/material-floating-button)), while a customisation file is provided (`mfb/_customisation.scss`) to override the defaults without editing the source files.
